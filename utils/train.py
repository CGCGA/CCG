...
def lightning_fit(
    ...
    if save_model:
        callbacks.append(
            ModelCheckpoint(
                dirpath='./checkpoints',
                # filename='model-{epoch:02d}-{val_metric:.2f}',  # 删除这一行
                monitor=metrics.val_metric,
                mode=metrics.eval_mode,
                save_last=True,
                filename=cktp_prefix + "{epoch}-{step}",  # 保留这一行
            )
        )
...
def up_lightning_fit(
    ...
    if save_model:
        ...
        callbacks.append(
            ModelCheckpoint(
                dirpath='./checkpoints/model' + formatted_time,
                # filename='model-{epoch:02d}-{val_metric:.2f}',  # 删除这一行
                monitor=metrics.val_metric,
                mode=metrics.eval_mode,
                save_last=True,
    ...
    print("upstream training over!!!")


#下游训练与评估
def down_lightning_fit(
    logger,
    model,
    data_module,
    metrics: EvalKit,
    ...
    trainer = Trainer(
     ...
     # 能起作用吗？
    model.model.freeze_all_except_weights()
